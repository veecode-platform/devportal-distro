...
3. `app-config.local.yaml` (from `VEECODE_APP_CONFIG` or manual)
4. `dynamic-plugins-root/app-config.dynamic-plugins.yaml` (generated by install script)
5. `app-config.{profile}.yaml` (if `VEECODE_PROFILE` is set)
